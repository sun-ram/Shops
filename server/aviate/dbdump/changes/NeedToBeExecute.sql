...
ADD COLUMN `REQUEST_ID` VARCHAR(32) NULL DEFAULT NULL AFTER `TRANSACTION_NO`;
 ...
 ----------28-12-2015 ----------- JAI ------- Product Offer ---------

ALTER TABLE `shopsbacker`.`product_offer` 
CHANGE COLUMN `FROM_DATE` `FROM_DATE` TIMESTAMP NULL DEFAULT NULL ,
CHANGE COLUMN `TODATE` `TODATE` TIMESTAMP NULL DEFAULT NULL ;

-- 29-12-2015 -- Kartheeswaran -- Sales order return and return line --

CREATE TABLE `sales_order_return` (
  `SALES_ORDER_RETURN_ID` varchar(32) NOT NULL,
  `SALES_ORDER_ID` varchar(32) NOT NULL,
  `RETURN_REASON` varchar(45) NOT NULL,
  `RETURN_TOTAL_AMOUNT` decimal(15,2) NOT NULL,
  `RETURN_STATUS` varchar(32) NOT NULL,
  `RETURN_TAX_AMOUNT` decimal(15,2) NOT NULL,
  `MERCHANT_ID` varchar(32) NOT NULL,
  `STORE_ID` varchar(32) NOT NULL,
  `CREATED` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `CREATEDBY` varchar(32) NOT NULL,
  ...
  UNIQUE KEY `SALES_ORDER_RETURN_ID_UNIQUE` (`SALES_ORDER_RETURN_ID`),
  KEY `SALES_RT_STORE_idx` (`STORE_ID`),
  KEY `SALES_RT_MERCHANT_idx` (`MERCHANT_ID`),
  KEY `SALES_RT_SALES_ORDER_idx` (`SALES_ORDER_ID`),
  CONSTRAINT `SALES_RT_MERCHANT` FOREIGN KEY (`MERCHANT_ID`) REFERENCES `merchant` (`MERCHANT_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `SALES_RT_SALES_ORDER` FOREIGN KEY (`SALES_ORDER_ID`) REFERENCES `sales_order` (`SALES_ORDER_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `SALES_RT_STORE` FOREIGN KEY (`STORE_ID`) REFERENCES `store` (`STORE_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
SELECT * FROM shopsbacker.sales_order_return;

CREATE TABLE `sales_order_return_line` (
  `SALES_ORDER_RETURN_LINE_ID` varchar(32) NOT NULL,
  `PRODUCT_ID` varchar(32) NOT NULL,
  `QTY` int(11) NOT NULL,
  `PRICE` decimal(15,2) NOT NULL,
  `RETURN_GROSS_AMOUNT` decimal(15,2) NOT NULL,
  `RETURN_NET_AMOUNT` decimal(15,2) NOT NULL,
  `ISACTIVE` char(1) NOT NULL DEFAULT 'Y',
  `CREATED` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `CREATEDBY` varchar(32) NOT NULL,
  `UPDATED` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
  ...
  `SALES_ORDER_RETURN_ID` varchar(32) NOT NULL,
  PRIMARY KEY (`SALES_ORDER_RETURN_LINE_ID`),
  UNIQUE KEY `SALES_ORDER_RETURN_LINE_ID_UNIQUE` (`SALES_ORDER_RETURN_LINE_ID`),
  KEY `SALES_RT_LI_PRODUCT_idx` (`PRODUCT_ID`),
  KEY `SALES_RT_LI_SALES_ORDER_LINE_idx` (`SALES_ORDER_LINE_ID`),
  KEY `SALES_RT_LI_SALES_ORDER_RETURN_idx` (`SALES_ORDER_RETURN_ID`),
  CONSTRAINT `SALES_RT_LI_SALES_ORDER_RETURN` FOREIGN KEY (`SALES_ORDER_RETURN_ID`) REFERENCES `sales_order_return` (`SALES_ORDER_RETURN_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `SALES_RT_LI_SALES_ORDER_LINE` FOREIGN KEY (`SALES_ORDER_LINE_ID`) REFERENCES `sales_order_line` (`SALES_ORDER_LINE_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `SALES_RT_LI_PRODUCT` FOREIGN KEY (`PRODUCT_ID`) REFERENCES `product` (`PRODUCT_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

---- 30-12-2015 -- JAI -- Product Offer Line --
...
CREATE TABLE `shopsbacker`.`billing` (
  ...
  KEY `FK_BILLING_SALES_ORDER_idx` (`SALES_ORDER_ID`),
  KEY `FK_BILLING_MERCHANT_idx` (`MERCHANT_ID`),
  KEY `FK_BILLING_STORE_idx` (`STORE_ID`),
  CONSTRAINT `FK_BILLING_MERCHANT` FOREIGN KEY (`MERCHANT_ID`) REFERENCES `merchant` (`MERCHANT_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_BILLING_SALES_ORDER` FOREIGN KEY (`SALES_ORDER_ID`) REFERENCES `sales_order` (`SALES_ORDER_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
-- --08-01-2016 -- Anbukkani Gajendran -- transcation table created.
CREATE TABLE `shopsbacker`.`transaction` (
  ...
  `STORE_ID` varchar(32) NOT NULL,
  `CREATED` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `CREADEDBY` varchar(32) NOT NULL,
  `UPDATED` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
  `UPDATEDBY` varchar(32) NOT NULL,
  `AMOUNT` decimal(15,2) DEFAULT NULL,
  PRIMARY KEY (`TRANSACTION_ID`),
  UNIQUE KEY `TRANSACTION_ID_UNIQUE` (`TRANSACTION_ID`),
  KEY `FK_TRANS_MERCHANT_idx` (`MERCHANT_ID`),
  KEY `FK_TRANS_STORE_idx` (`STORE_ID`),
  CONSTRAINT `FK_TRANS_MERCHANT` FOREIGN KEY (`MERCHANT_ID`) REFERENCES `merchant` (`MERCHANT_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
-- --13-01-2016 -- Kathir-- For create billing payment table --------------- --
CREATE TABLE `shopsbacker`.`billing_payment` (
  `BILLING_PAYMENT_ID` varchar(32) NOT NULL,
  `MERCHANT_ID` varchar(32) NOT NULL,
  ...
  UNIQUE KEY `BILLING_PAYMENT_ID_UNIQUE` (`BILLING_PAYMENT_ID`),
  KEY `FK_BILL_PAY_MERCHANT_idx` (`MERCHANT_ID`),
  CONSTRAINT `FK_BILL_PAY_MERCHANT` FOREIGN KEY (`MERCHANT_ID`) REFERENCES `merchant` (`MERCHANT_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --13-01-2016 -- Kathir-- updating in billing table --------------- --
ALTER TABLE `shopsbacker`.`billing` 
ADD COLUMN `BILLING_PAYMENT_ID` VARCHAR(45) NULL AFTER `PAID_DATE`,
ADD INDEX `FK_BILLING_BILLING_PAYMENT_idx` (`BILLING_PAYMENT_ID` ASC);
ALTER TABLE `shopsbacker`.`billing` 
ADD CONSTRAINT `FK_BILLING_BILLING_PAYMENT`
  FOREIGN KEY (`BILLING_PAYMENT_ID`)
  REFERENCES `shopsbacker`.`billing_payment` (`BILLING_PAYMENT_ID`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
  
  -- --13-01-2016 -- Kathir-- updating in transection table --------------- --
  ALTER TABLE `shopsbacker`.`transaction` 
DROP FOREIGN KEY `FK_TRANS_STORE`;
ALTER TABLE `shopsbacker`.`transaction` 
CHANGE COLUMN `STORE_ID` `STORE_ID` VARCHAR(32) NULL ;
ALTER TABLE `shopsbacker`.`transaction` 
ADD CONSTRAINT `FK_TRANS_STORE`
  FOREIGN KEY (`STORE_ID`)
  REFERENCES `shopsbacker`.`store` (`STORE_ID`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
