...
--- 24-12-2015 ------- JAI -------------------------- Discount Table StoreId Key Added ------
ALTER TABLE `shopsbacker`.`discount_product` 
ADD COLUMN `MERCHANT_ID` VARCHAR(32) NOT NULL AFTER `UPDATEDBY`;

ALTER TABLE `shopsbacker`.`discount_product` 
ADD INDEX `fk_DISC_PROD_MERCHANT_idx` (`MERCHANT_ID` ASC);
ALTER TABLE `shopsbacker`.`discount_product` 
ADD CONSTRAINT `fk_DISC_PROD_MERCHANT`
  FOREIGN KEY (`MERCHANT_ID`)
  REFERENCES `shopsbacker`.`merchant` (`MERCHANT_ID`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
...
ALTER TABLE `shopsbacker`.`discount` 
DROP INDEX `DISCOUNT_NAME_UNIQUE` ,
ADD UNIQUE INDEX `DISCOUNT_NAME_UNIQUE` (`NAME` ASC, `STORE_ID` ASC);
