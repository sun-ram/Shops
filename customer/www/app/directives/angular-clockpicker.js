...
.directive('clockpicker', function() {
	return {
		...
		controller: function ($scope,$rootScope,$mdDialog,toastr,$localStorage,AppServices,$interval,$timeout) {
			...
			$scope.setUpdatedTime = function(){
				if($rootScope.timeSelected != true){
					$timeout(function() {
						$scope.$apply(function(){
							$scope.datetime = new Date();
							$scope.datetime.setMinutes($scope.datetime.getMinutes()+60);
			...
			$interval($scope.setUpdatedTime,5000);
		}
	};
})
