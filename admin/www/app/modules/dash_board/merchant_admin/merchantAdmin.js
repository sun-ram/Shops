...
//						.attr("y", bandPos[1]) ;
		...
		function getQualityStats (times){
			...
			for(;($scope.stores.Books && i<$scope.stores.Books.length);i++){
				tmpArray = {};
				red=green=yellow=0;
...
//		};
		...
		var callAllwebservices = function (){
			...
			sendHttpRequest('salesOrder').then(function (data) {
				...
				data.Books = _.reject(data.Books, function(book){ return (book.STATUS != 'Delivered' || book.MERCHANT_ID != $rootScope.user.merchantId) ;});
