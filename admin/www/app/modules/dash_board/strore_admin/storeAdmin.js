aviateAdmin.controller("storeDashboardCtrl", ['$scope', '$localStorage', '$location', '$state', '$mdDialog', 'EmployeeService', 'toastr', 'CONSTANT', 'myConfig', '$rootScope', 'CommonServices', 'StoreServices', 'api', '$http', '$q',
    function ($scope, $localStorage, $location, $state, $mdDialog, EmployeeService, toastr, CONSTANT, myConfig, $rootScope, CommonServices, StoreServices, api, $http, $q) {
        ...
        function groupedBarChart() {
            var data = [];
            var i = 0;
            len = $scope.qualirtStatsRecords.length;
            var tmpGroupObj = {};
            for (; i < len; i++) {
                tmpGroupObj = {};
        ...
        function calculateCodSalesOrder(data) {
            var codSalesValue = 0,
                cardSalesValue = 0;
            _.each(data.Books, function (order) {
                if (order.STORE_ID == $rootScope.user.storeId && order.PAYMENT_METHOD == "COD") {
                    codSalesValue = codSalesValue + order.NET_AMOUNT;
                } else {
                    cardSalesValue = cardSalesValue + order.NET_AMOUNT;
                }
            });
            var drawRadialProgressOfCODSales = function (id, label, value) {
                radialProgress($(id)[0])
                    .maxValue(codSalesValue + cardSalesValue)
                    .diameter(125)
            ...
            drawRadialProgressOfCODSales('#cod-pay', 'COD-Sales', codSalesValue);
            drawRadialProgressOfCODSales('#card-pay', 'Card-Sales', cardSalesValue);
        }
        ...
        $scope.proceedSalesOrder = function (callback) {
            ...
            sendHttpRequest('salesOrder').then(function (data) {
                data.Books = _.reject(data.Books, function (book) {
                    return ($rootScope.user.storeId != book.STORE_ID);
                });
